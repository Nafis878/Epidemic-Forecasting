...
% Replace neurips_stub with the official neurips_2026.sty before submission.
...
forecaster on the paired common-mask benchmark at season-unweighted WIS \msEnsTrimmedWIS{}.
It is significantly better than the \textbf{median ensemble} (\msEnsMedianWIS{}; the default
deployed by the CDC FluSight and COVID-19 Forecast Hubs) under both clustered bootstrap and
Holm-adjusted Diebold--Mariano tests, and better than the best single model (PatchTST
\msPatchTSTWIS{}) under clustered bootstrap and BH-adjusted DM but not Holm-adjusted DM.
Conversely, a \textbf{phase-gated, online-learning, conformal hybrid} we designed to exploit
...
\begin{table}[t]
\centering
\caption{Multi-season base-model leaderboard (mean WIS; lower is better; paired common forecast mask).
MIST wins only its development season 2023-24 and is third of four on the season-unweighted mean.
Generated from \texttt{results/season\_leaderboard\_common.csv}.}
\label{tab:base}
\input{season_base_auto.tex}
\end{table}
...
all base models, ensembles, and hybrid ablations on the paired common forecast mask (numbers come
exclusively from a \texttt{-{}-full} run; quick-pipeline numbers validate plumbing only and are never
reported). The paired mask contains 18{,}018 forecast cells and drops 414 incomplete 2024-25 cells
where at least one model did not produce an aligned forecast. The strongest forecaster
...
ensemble} (\msEnsMedianWIS{}; the hubs' deployed default) under clustered bootstrap and Holm-DM,
and it beats the best single model (PatchTST \msPatchTSTWIS{}) under clustered bootstrap and
BH-adjusted DM, although the PatchTST comparison does not survive Holm correction. It also beats
the plain mean ensemble (\msEnsMeanWIS{}, dragged down by weak members that trimming discards).
We report both corrected tests (\texttt{tables/bootstrap\_ci\_common.csv},
\texttt{tables/multiseason\_dm\_adjusted\_common.csv}). The lesson is practical:
\emph{robust} aggregation,
...
\begin{table}[t]
\centering
\caption{Multi-season paired leaderboard: base models, ensembles, and hybrid ablations
(season-unweighted WIS on the common forecast mask; the winning combiner in bold). Generated from
\texttt{results/multiseason\_common\_summary.csv} on a \texttt{-{}-full} run; a quick run leaves a
